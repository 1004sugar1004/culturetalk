// 전역 변수
let characters = charactersData; // data.js에서 로드
let messages = messagesData; // data.js에서 로드
...
function init() {
    try {
        // 데이터는 data.js에서 이미 로드됨
        setupEventListeners();
        console.log('앱 초기화 완료:', characters.length, '명의 캐릭터');
    } catch (error) {
...
// 데이터는 data.js에서 직접 로드되므로 이 함수들은 더 이상 필요 없음
